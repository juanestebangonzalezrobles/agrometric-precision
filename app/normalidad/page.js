...
import { ComposedChart, BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, LineChart, Line, ScatterChart, Scatter, ReferenceLine } from 'recharts';
import { normalityTest, histogram, optimizeBoxCox, aguacatePeso, aloeAltura } from '../../lib/data';

// Simple client‑side ErrorBoundary
class ErrorBoundary extends React.Component {
  ...
  render() {
    if (this.state.hasError) {
      return (
        <div className="error-box" style={{ padding: '20px', background: 'rgba(255,0,0,0.1)', border: '1px solid #ef4444' }}>
...
export default function NormalidadPage() {
  ...
  const runAnalysis = (values, applyBoxCox = false) => {
    ...
    const qq = r.sorted.map((val, i) => {
      ...
      const theoreticalZ = p < 0.5 ? -zAbs : zAbs;
      return { theoretical: +theoreticalZ.toFixed(3), sample: +((val - r.mean) / r.s).toFixed(3) };
    });
        ...
        <div className="page-content fade-in">
          ...
          {result && (
            <div className="grid-3" style={{ marginBottom: 16 }}>
              {[{ label: 'N (Datos)', val: result.n }, { label: 'Media (X̄)', val: result.mean.toFixed(4) }, { label: 'Desv. Estándar (S)', val: result.s.toFixed(4) }, { label: 'Asimetría', val: result.skew.toFixed(4), color: Math.abs(result.skew) > 1 ? 'yellow' : 'green' }, { label: 'Curtosis', val: result.kurt.toFixed(4), color: Math.abs(result.kurt) > 1 ? 'yellow' : 'green' }, { label: 'Est. A² (And‑Darling)', val: result.A2.toFixed(4), color: result.normal ? 'green' : 'red' }].map((s, i) => (
                <div key={i} className="card" style={{ textAlign: 'center', padding: '12px' }}>
                  ...
                  <div style={{ fontSize: 20, fontWeight: 800, fontFamily: 'JetBrains Mono', color: { green: 'var(--green-light)', red: '#ef4444', yellow: '#eab308' }[s.color] || 'var(--green-light)' }}>{s.val}</div>
                </div>
              ))}
            </div>
          )}
            ...
            <div className="chart-wrapper">
              ...
              <ResponsiveContainer width="100%" height={280}>
                <ComposedChart data={histData} margin={{ top: 15, right: 20, left: 10, bottom: 20 }}>
                  ...
                  <YAxis stroke="var(--text-muted)" tick={{ fontSize: 11, fill: 'var(--text-muted)' }} domain={[0, Math.max(...histData.map(h => Math.max(h.count || 0, h.normal || 0))) * 1.1]} allowDataOverflow />
            ...
            <div className="chart-wrapper">
              ...
              <ResponsiveContainer width="100%" height={280}>
                <ScatterChart margin={{ top: 15, right: 30, left: 10, bottom: 15 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="var(--border)" />
                  <XAxis dataKey="theoretical" type="number" name="Cuantiles Teóricos (Z)" stroke="var(--text-muted)" tick={{ fontSize: 11, fill: 'var(--text-muted)' }} domain={[Math.min(...qqData.map(d => d.theoretical)) - 0.5, Math.max(...qqData.map(d => d.theoretical)) + 0.5]} allowDataOverflow label={{ value: 'Cuantiles Teóricos', position: 'insideBottom', offset: -5, fill: 'var(--text-muted)', fontSize: 11 }} />
                  <YAxis dataKey="sample" type="number" name="Cuantiles Muestrales" stroke="var(--text-muted)" tick={{ fontSize: 11, fill: 'var(--text-muted)' }} domain={[Math.min(...qqData.map(d => d.sample)) - 0.5, Math.max(...qqData.map(d => d.sample)) + 0.5]} allowDataOverflow label={{ value: 'Cuantiles Muestrales', angle: -90, position: 'insideLeft', fill: 'var(--text-muted)', fontSize: 11 }} />
                  ...
                  <ReferenceLine segment={[{ x: Math.min(...qqData.map(d => d.theoretical)) - 0.5, y: Math.min(...qqData.map(d => d.sample)) - 0.5 }, { x: Math.max(...qqData.map(d => d.theoretical)) + 0.5, y: Math.max(...qqData.map(d => d.sample)) + 0.5 }]} stroke="#ef4444" strokeWidth={1.5} strokeDasharray="3 3" label={{ value: 'Diagonal Teórica', fill: '#ef4444', fontSize: 10, position: 'insideTopLeft' }} />
