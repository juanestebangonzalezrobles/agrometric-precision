...
export default function Dashboard() {
  ...
  useEffect(() => {
    try {
      ...
      // Limpieza proactiva de datos demo y sembrados (seeded)
      if (loaded.some(r => r && (r.isDemo || r.id?.startsWith('demo_') || r.id?.startsWith('seeded_')))) {
        loaded = loaded.filter(r => r && !r.isDemo && !r.id?.startsWith('demo_') && !r.id?.startsWith('seeded_'));
