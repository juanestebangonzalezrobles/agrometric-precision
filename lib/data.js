...
export function calcCapability(subgrupos, lse, lie) {
  ...
  const variance = allValues.reduce((a, b) => a + (b - mean) ** 2, 0) / (n - 1);
  const sigma = Math.sqrt(variance);
  const Cp = (lse - lie) / (6 * sigma);
  const Cpu = (lse - mean) / (3 * sigma);
  const Cpl = (mean - lie) / (3 * sigma);
  const Cpk = Math.min(Cpu, Cpl);
  const Pp = (lse - lie) / (6 * sigma);
  const Ppk = Math.min((lse - mean) / (3 * sigma), (mean - lie) / (3 * sigma));
  return { mean, sigma, Cp, Cpk, Cpu, Cpl, Pp, Ppk, allValues, lse, lie };
}
...
export { aguacatePeso, aloeAltura, manzanillaP, tomateDefectos, A2, D3, D4, A3, B3, B4, c4Factor };
